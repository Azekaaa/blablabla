...
def _deal_line(deal: dict, show_days: str = "inactive") -> str:
    """Format a single deal line for the report."""
...
def build_full_report(analytics: CRMAnalytics) -> str:
    """Build a complete CRM report for Telegram."""
    ...
    if analytics.inactive_deals:
        lines.append("")
        lines.append(
            f"🕐 *Без активности >{settings.inactive_days_threshold}д* "
            f"({len(analytics.inactive_deals)}):"
        )
        for deal in analytics.inactive_deals[:5]:
            lines.append(_deal_line(deal, "inactive"))
        if len(analytics.inactive_deals) > 5:
            lines.append(f"  _...и ещё {len(analytics.inactive_deals) - 5} сделок_")
    else:
    ...
    if analytics.deals_without_tasks:
        ...
        if len(analytics.deals_without_tasks) > 5:
            lines.append(f"  _...и ещё {len(analytics.deals_without_tasks) - 5} сделок_")
    else:
    ...
    if analytics.stuck_stage_deals:
        lines.append("")
        lines.append(
            f"🔒 *Застряли на этапе >{settings.stuck_stage_days_threshold}д* "
            f"({len(analytics.stuck_stage_deals)}):"
        )
        for deal in analytics.stuck_stage_deals[:5]:
            lines.append(_deal_line(deal, "stage"))
        if len(analytics.stuck_stage_deals) > 5:
            lines.append(f"  _...и ещё {len(analytics.stuck_stage_deals) - 5} сделок_")
    else:
    ...
    for i, ms in enumerate(analytics.manager_stats[:5]):
        ...
        lines.append(
            f"   📁 {ms.deal_count} сд | 💰 {amount_str}{problem_str}"
        )
    ...
    total_deals = analytics.total_active_deals or 1  # avoid division by zero
    for ss in analytics.stage_stats[:8]:
        ...
        lines.append(
            f"   {ss.count} сделок ({pct:.0f}%) | {_fmt_money(ss.total_amount, currency)}"
        )

    # ── Конверсия ───────────────────────────────
    if analytics.won_deals_today > 0 or analytics.new_deals_today > 0:
        lines.append("")
        lines.append("━━━━━━━━━━━━━━━━━━━━")
        lines.append("📊 *КОНВЕРСИЯ (сегодня)*")
        base = analytics.new_deals_today + analytics.closed_deals_today
        conv = analytics.won_deals_today / base * 100 if base > 0 else 0
        lines.append(f"🎯 Успешных из закрытых: *{conv:.0f}%*")
...
def build_stats_report(analytics: CRMAnalytics) -> str:
    ...
    for ss in analytics.stage_stats:
        ...
        lines.append(
            f"  • {ss.stage_name}: {ss.count} ({pct:.0f}%) — {_fmt_money(ss.total_amount, currency)}"
        )
...
def _generate_conclusion(analytics: CRMAnalytics) -> str:
    """Generate a textual conclusion based on analytics data."""
    ...
    if analytics.total_problem_deals == 0:
        ...
        conclusions.append(f"⚠️ Есть {analytics.total_problem_deals} проблемных сделки — требуют внимания.")
    else:
    ...
    if len(analytics.deals_without_tasks) > 5:
        conclusions.append(
            f"📋 {len(analytics.deals_without_tasks)} сделок без задач — риск потери клиентов."
        )

    # Top manager
    if analytics.manager_stats:
        top = analytics.manager_stats[0]
        from bot.reports.formatter import _fmt_money
        conclusions.append(
            f"🏆 Лидер: {top.name} — {_fmt_money(top.total_amount, analytics.currency)} в работе."
        )
